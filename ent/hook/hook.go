...
// The RocketFunc type is an adapter to allow the use of ordinary
// function as Rocket mutator.
type RocketFunc func(context.Context, *ent.RocketMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RocketFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RocketMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RocketMutation", m)
}
