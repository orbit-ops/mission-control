...
func (rq *RequestQuery) QueryMission() *MissionQuery {
...
func (rq *RequestQuery) WithMission(opts ...func(*MissionQuery)) *RequestQuery {
	query := (&MissionClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withMission = query
	return rq
}
