...
type RocketDelete struct {
	config
	hooks    []Hook
	mutation *RocketMutation
}

// Where appends a list predicates to the RocketDelete builder.
func (rd *RocketDelete) Where(ps ...predicate.Rocket) *RocketDelete {
	rd.mutation.Where(ps...)
	return rd
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (rd *RocketDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, rd.sqlExec, rd.mutation, rd.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (rd *RocketDelete) ExecX(ctx context.Context) int {
	n, err := rd.Exec(ctx)
	if err != nil {
		panic(err)
	}
...
type RocketDeleteOne struct {
	rd *RocketDelete
}

// Where appends a list predicates to the RocketDelete builder.
func (rdo *RocketDeleteOne) Where(ps ...predicate.Rocket) *RocketDeleteOne {
	rdo.rd.mutation.Where(ps...)
	return rdo
}

// Exec executes the deletion query.
func (rdo *RocketDeleteOne) Exec(ctx context.Context) error {
	n, err := rdo.rd.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{rocket.Label}
	default:
		return nil
	}
}
...
// ExecX is like Exec, but panics if an error occurs.
func (rdo *RocketDeleteOne) ExecX(ctx context.Context) {
